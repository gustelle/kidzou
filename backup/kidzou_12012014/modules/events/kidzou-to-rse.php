...
/**
* la gestion des evenements se fait par l'intégration du plugin "Really Simple Events"
* ce plugin a été customisé pour les besoins de KIDZOU:
* 	- intégration avec les fiches de l'annuaire (connections)
...
function get_teaser_events_xdays( $days=7, $limit = 3 )
{
	...
	$eventQuery  = "SELECT * FROM $table_name WHERE start_date <= ADDDATE( CURDATE( ) , $days ) AND ( start_date >= now() OR end_date >= now() )  ORDER BY featured DESC, start_date ASC ";
...
function get_upcoming_events_xdays( $days=7 )
{
	...
	if ( false === ( $events = get_transient( 'kz_upcoming_events_'.$days.'days' ) ) ) {
		...
		while ($i<$days) {
	    	...
	    	$eventQuery  = "SELECT *, ADDDATE( CURDATE( ) , $i ) as the_date FROM $table_name WHERE start_date <= ADDDATE( CURDATE( ) , $i ) AND ( start_date >= now() OR end_date >= now() ) ORDER BY featured DESC, start_date ASC ";
...
function get_upcoming_events_by_customer( $id )
{
	...
	if ($id!="" && is_int($id)){
		...
		//$wpdb->show_errors();
		$eventQuery  = "SELECT * FROM $table_name WHERE customer=$id AND (start_date >= now() OR end_date >= now()) ORDER BY featured DESC, start_date ASC ";
		$res = $wpdb->get_results( $eventQuery );
		//$wpdb->print_error();
...
function get_upcoming_events_by_post($post_id)
{
	...
	// if ( false === ( $results = get_transient( 'kz_upcoming_events_by_post_'.$post_id ) ) ) {
		...
		if ($connections_id!='')
		{
			global $wpdb;
			$table_name = $wpdb->prefix . HC_RSE_TABLE_NAME;
			$eventQuery  = "SELECT * FROM $table_name as r WHERE ( ( r.start_date >= NOW() AND r.start_date <= ADDDATE( CURDATE( ) , 14 ) ) OR ( r.end_date >= NOW() AND r.end_date <= ADDDATE( CURDATE( ) , 14 ) ) ) and r.connections_id='$connections_id' ORDER BY r.featured DESC,r.start_date ASC LIMIT 0,3";
