(function($){
	var menu_flip_speed = 200,
		...
		// $et_mobile_nav_ul = $('#mobile_nav ul'),
		$main_menu = $('.rubriques ul.nav'),
		$transverse_menu = $('.transverse ul.nav'),
		$transverse_menu_li = $('.transverse ul.nav li'),
		$featured = $('#featured'),
	...
	$(document).ready(function(){
		...
		$transverse_menu.find('>li').each( function(){
			var $this_li = $(this),
				li_text = $this_li.find('>a').html();
		...
		$transverse_menu.find('>li').hover( function(){
			$(this).addClass( 'et_hover' );
		}, function(){
			$(this).removeClass( 'et_hover' );
		} );
		...
		if ( ! $('html#ie7').length ){
			...
			$transverse_menu.find('>li>a').live({
				mouseenter: function(){
					if ( ! $(this).parent('li').hasClass('current-menu-item') )
						$(this).find('span.main_text').animate( { 'marginTop' : '-44px' }, menu_flip_speed );
				},
		...
		$transverse_menu_li.clone().appendTo( $cloned_nav ); 
