...
/**
 KIDZOU : microdata sur getNameBlock + getImage + getAddressBlock + getPhoneNumberBlock
*/
...
class cnOutput extends cnEntry
{
		...
		switch ( $atts['image'] ) {
			case 'photo':
				...
				if ( ! empty( $links ) ) {
					...
					$anchorStart = '<a itemprop="logo" href="' . $link->url . '"' . ( ( empty( $link->target ) ? '' : ' target="' . $link->target . '"' ) ) . ( ( empty( $link->followString ) ? '' : ' rel="' . $link->followString . '"' ) ) . '>';
				}
			...
			case 'logo':
				...
				if ( ! empty( $links ) ) {
					...
					$anchorStart = '<a itemprop="logo" href="' . $link->getWebsites . '"' . ( ( empty( $link->target ) ? '' : ' target="' . $link->target . '"' ) ) . ( ( empty( $link->followString ) ? '' : ' rel="' . $link->followString . '"' ) ) . '>';
				}
	...
	public function getNameBlock( $atts = array() ) {
		...
		switch ( $this->getEntryType() ) {
			...
			case 'organization':
				$out = '<span class="org fn" itemprop="name">' . $this->getOrganization() . '</span>';
				break;
	...
	public function getAddressBlock( $atts = array() , $cached = TRUE ) {
		...
		foreach ( $addresses as $address ) {
			...
			$out .= "\n" . '<span class="adr" itemprop="location" itemscope itemtype="http://schema.org/PostalAddress">';

			( empty( $address->name ) ) ? $replace[] = '' : $replace[] = '<span class="address-name">' . $address->name . '</span>';
			( empty( $address->line_1 ) ) ? $replace[] = '' : $replace[] = '<span class="street-address" itemprop="streetAddress">' . $address->line_1 . '</span>';
			...
			( empty( $address->city ) ) ? $replace[] = '' : $replace[] = '<span class="locality" itemprop="addressLocality">' . $address->city . '</span>';
			( empty( $address->state ) ) ? $replace[] = '' : $replace[] = '<span class="region">' . $address->state . '</span>';
			( empty( $address->zipcode ) ) ? $replace[] = '' : $replace[] = '<span class="postal-code" itemprop="postalCode">' . $address->zipcode . '</span>';

			( empty( $address->country ) ) ? $replace[] = '' : $replace[] = '<span class="country-name">' . $address->country . '</span>';

			if ( ! empty( $address->latitude ) || ! empty( $address->longitude ) ) {
				$replace[] = '<span class="geo">' .
					( ( empty( $address->latitude ) ) ? '' : '<span class="latitude" title="' . $address->latitude . '"><span class="cn-label">' . __( 'Latitude', 'connections' ) . ': </span><span>' . $address->latitude . '</span></span>' ) .
					( ( empty( $address->longitude ) ) ? '' : '<span class="longitude" title="' . $address->longitude . '"><span class="cn-label">' . __( 'Longitude', 'connections' ) . ': </span><span>' . $address->longitude . '</span></span>' ) .
					'</span>';
			}
		...
		foreach ( $phoneNumbers as $phone ) {
			...
			if ( empty( $phone->number ) ) {
				...
				if ( $connections->settings->get( 'connections', 'connections_link', 'phone' ) ) {
					$replace[] = '<a class="value" itemprop="telephone" href="tel:' . $phone->number . '" value="' . preg_replace( '/[^0-9]/', '', $phone->number ) . '">' . $phone->number . '</a>';
				} else {
					$replace[] = '<span class="value" itemprop="telephone">' . $phone->number . '</span>';
				}
