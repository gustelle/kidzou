...
function kz_clients() 
{
	...
	wp_enqueue_script('kidzou-admin', WP_PLUGIN_URL.'/kidzou/js/kidzou-admin.js'	,array("jquery","ko"), '0.1', true);
...
function kz_options()
{
	...
	//email de notification des evenements à publier
	$events_notify 			 = get_option("kz_events_notify");
	...
	if ($_POST['submit'])
	{
		...
		if ( get_option( "kz_events_notify" ) != $mail )
		    update_option( "kz_events_notify", $mail );
		else
		    add_option( "kz_events_notify", $mail );
...
<div class="wrap">
	 <form method="POST" action="<?php echo $_SERVER['REQUEST_URI']; ?>" >
	 	...
	 	<p>
	 		<input type="email" value="<?php echo $mail; ?>" id="events" name="events_notify">
	 		<span style="padding-left:5px;">Personne &agrave; informer des demandes de publication d&apos;&eacute;v&eacute;nement</span>
	 	</p>
