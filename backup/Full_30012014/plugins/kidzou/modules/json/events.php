...
class JSON_API_Events_Controller {
	...
	function saveEvent()
	{
		...
		$table_events = $wpdb->prefix . "reallysimpleevents";
	 	...
	 	//le user est-il un client ?
	 	//si oui, recuperer la fiche Connections du client
	 	$uid = $wpdb->get_var( "SELECT customer_id FROM wp_clients_users WHERE user_id=$current_user->ID" );
		if ($uid && intval($uid)>0) {
			$data["customer"] = $uid;
		}
		...
		$table_events = $wpdb->prefix . "reallysimpleevents";
	 	...
	 	if ($id!='' && intval($id)>0)
	 	{
	 		...
	 		if ($event!=null && count($event)==1)
	 		{
	 			...
	 			//$event->image (http://localhost/wordpress/wp-content/uploads/events/2014/01/1/bonhommes.jpg)
	 			$image_url = $event[0]['image']; 
	...
	function attachImage ()
	{
		...
		global $current_user;
      	...
      	require_once( plugin_dir_path( __FILE__ ) . '../events/kidzou-to-rse.php' );

		$uploaded_file = $_FILES['file'];

		add_filter('wp_handle_upload_prefilter', 'event_pre_upload', 2);
		add_filter('wp_handle_upload', 'event_post_upload', 2);

		$moved = wp_handle_upload($uploaded_file, array('test_form' => false));		//print_r ($moved);
		if (isset($moved['file'])) {
