...
$hc_rse_db_version = "1.4-Kidzou.27";
...
function hc_rse_display_link($link){
	$html = '<a href="{{link}}" title="{{title}}" target="_blank" alt="{{title}}">{{title}}</a>';
...
function hc_rse_plugin_install(){
	...
	$sql = "CREATE TABLE $table_name (
        ...
        extra_venue int(1) DEFAULT NULL,
        modification_date timestamp NOT NULL DEFAULT '2013-01-01 00:00:00',
        status ENUM('draft', 'requested', 'approved') DEFAULT 'draft',
        customer mediumint(9) NOT NULL DEFAULT 0,
...
function hc_rse_build_admin_menu(){
	...
	add_submenu_page( 'hc_rse_event' ,
		              __( 'Ev&egrave;nements &agrave; valider' , 'hc_rse' ) ,
		              __( 'Ev&egrave;nements &agrave; valider' , 'hc_rse' ) ,
		              $user_capability ,
		              'kz_publishRequests',
		              'kz_publishRequests'
...
function kz_publishRequests(){

	?>
	<h1>Demandes de publication</h1>
	<?php
	...
	wp_enqueue_script('kidzou-publishRequests', WP_PLUGIN_URL.'/kidzou/js/kidzou-publishRequests.js'	,array("jquery","ko"), '0.1', true);
	...
	wp_localize_script('kidzou-publishRequests', 'kidzou_jsvars', array(
	...
	require_once WP_PLUGIN_DIR . '/kidzou/kidzou-admin_publishRequests.php';
}
