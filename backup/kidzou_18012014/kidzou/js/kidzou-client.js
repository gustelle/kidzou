...
var kidzouAdmin = (function() { //havre de paix
	...
	function ClientsViewModel() {
	    ...
	    self.doSaveUsers = function(form) {
	    	...
	    	//console.log("self.selectedUsers() " + self.selectedUsers());
	    	...
	    	//pas de vérif sur les users secondaires
	    ...
	    self.formatEvent = function(ev) {  
	    	var status	 	= ev.status; 
	   		...
	   		return "<span class='" + (status==="approved" ? "validated" : "draft") + "'>" + ev.title + " <span class='date'>(" + start.format("DD MMM YYYY") + ", " + moment.duration(diff, "hours").humanize() + ")</span></span>"; 
	    };
	    self.selectedEventId = function(e) { 
	    	return e.id+"|"+e.title+"|"+e.status+"|"+e.start_date+"|"+e.end_date;  
	    };
	    self.initSelectedEvents = function (element, callback) {
	    	var data = [];
			ko.utils.arrayForEach(self.attachedEvents().split(","), function(item) {
				var pieces = item.split("|");
				data.push({id: pieces[0], title: pieces[1], status: pieces[2], start_date:pieces[3], end_date: pieces[4]});
			});
	        callback(data);
	    };
		...
		self.detachEvent = function(data) {
			...
			var customer_id = self.chosenClientData().id();
		...
		self.getAllEvents = function() {
			...
			jQuery.getJSON(kidzou_jsvars.api_getEventsByClientID, { id: client.id() }, function (data) {
		...
		function getMoreEvents () {
			...
			jQuery.getJSON(kidzou_jsvars.getEventsByClientID, { id: client.id(), page : self.currentPage(), filters: ko.toJS(self.filters), index:self.chosenClientEvents().length  }, function (data) {
