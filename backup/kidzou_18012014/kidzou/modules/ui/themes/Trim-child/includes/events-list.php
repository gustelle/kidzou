		...
		?>
			...
			<article 	class="selectableEvent shadow-light events entry clearfix <?php if (is_event_today($event) && !$day_after) {echo 'today';} ?> <?php if ($agenda) {echo 'agenda';} ?> <?php if ($day_after) {echo 'day-after';} ?>" 
				...
				<?php if ($event->address!='') { ?>
					<meta itemprop="location" content="<?php echo $event->address; ?>">
				<?php } ?>
				...
				<header class="entry-header">
					...
					<?php if ($event->venue!='') { ?>
						<span><span class="booticon-map-marker"></span><?php echo $event->venue; ?></span>
					<?php } ?>
				</header><!-- .entry-header -->

				<!--section class="entry-content" id="content-< ? php echo $event->id; ?>" -->
					<!--p>< ? php echo stripslashes( limit_text($event->extra_info,30) ); ?><p-->
