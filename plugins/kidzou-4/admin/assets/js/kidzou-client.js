	...
	function ClientsViewModel() {
	  ...
	  //           	if (data) {
	    ...
	    self.formatEvent = function(ev) {  
	    	...
	    	if (typeof ev.custom_fields.kz_event_start_date != "undefined")
	   			start  	= moment(ev.custom_fields.kz_event_start_date, "YYYY-MM-DD HH:mm:ss");
	   		if (typeof ev.custom_fields.kz_event_end_date != "undefined")
	   			end 		= moment(ev.custom_fields.kz_event_end_date, "YYYY-MM-DD HH:mm:ss");
	   		
	   		if (start!='' && end!='') {
	   			diff 		= start.diff(end, 'hours'); // 1
	   			duration = " <span class='date'>(" + start.format("DD MMM YYYY") + ", " + moment.duration(diff, "hours").humanize() + ")</span>";
	   		}
