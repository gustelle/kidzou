...
var GooglePlacesAPI = {
  ...
  _handlePredictions: function(def, abbreviatedPlaceResults, status) {  //console.debug('_handlePredictions',abbreviatedPlaceResults );
  ...
  _handleDetails: function(def, displayText, placeResult, status) { //console.debug('_handleDetails', def, displayText, placeResult, status);
    if (status !== google.maps.places.PlacesServiceStatus.OK) {
      def.reject(status);
      return;
    }
  ...
  getPredictions: function(searchTerm, requestParams) { //console.debug('getPredictions', searchTerm);
  ...
  getDetails: function(abbreviatedPlaceResult) { //console.debug("getDetails", abbreviatedPlaceResult)
    return this.deferred.then($.proxy(function() {
      var deferred = new $.Deferred();
...
Selectize.define(pluginName, function(options) { 
  ...
  this.setup = (function() {
    ...
    return function() {
      ...
      // Initialize
      GooglePlacesAPI.init(this.$input[0]); //console.debug('this.$input[0]', this.$input[0]);

      self.settings.load = function(query, callback) { //console.debug('load', query, callback)
        ...
        GooglePlacesAPI.getPredictions(query, options.requestParams)
          .done(function(aprs) {
            ...
            callback(results); //console.debug('callback', callback);
          })
      ...
      self.settings.render.option = function(data) { //console.debug('render', data);
        ...
        m = re.exec(data.text); //console.debug('m', m)
  ...
  this.onOptionSelect = (function() { 
    ...
    return function(e) {
      ...
      GooglePlacesAPI.getDetails(data)
        .done(function(placeResult) { //console.debug('getDetails', placeResult)
          var val = null; 
          ...
          self.removeOption(value); //console.debug('self.removeOption(value)', self, value);
          self.addOption(data);
