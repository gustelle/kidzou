...
Controller Author: Matt Berg + Kidzou 
...
class JSON_API_Auth_Controller {
	...
	public function generate_auth_cookie() {
		...
		if (!$json_api->query->username) {
			$json_api->error("merci de renseigner l&apos;identifiant.");
		}

		if (!$json_api->query->password) {
			$json_api->error("merci de renseigner le mot de passe.");
		}		

    	$user = wp_authenticate($json_api->query->username, $json_api->query->password);
    	if (is_wp_error($user)) {
    		$json_api->error("Identifiant ou mot de passe invalide", 'error', '401');
    	...
    	$remember = $json_api->query->rememberme;
    	...
    	// if ($json_api->query->referer!='')
    	// 	$redirect = $json_api->query->referer;
    	// else
    		$redirect = site_url().'/espace-membre/';

    	if (is_array($user->roles) && in_array('administrator', $user->roles)) $redirect = admin_url();
