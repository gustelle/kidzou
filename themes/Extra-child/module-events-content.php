	...
	?>
		<li>
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'title-thumb-hover' ); ?>>
				...
				?>
				<div class="post-content">
					<h3 class="entry-title"><a href="<?php the_permalink(); ?>" data-hover-color="<?php echo esc_attr( $color ); ?>"><?php the_title(); ?></a>
					</h3>
					<p class="post-meta vcard">
					<?php 
						if (Kidzou_Events::isTypeEvent()) {
							setlocale(LC_TIME, "fr_FR");
							$dates = Kidzou_Events::getEventDates();
							...
							$end_day = explode(" ", $dates['end_date']);
							if ($start_day==$end_day) {
								echo '<i class="fa fa-calendar"></i> Le '.strftime("%d %B", strtotime($dates['start_date']));
							} else {
								echo '<i class="fa fa-calendar"></i> Du '.strftime("%d %B", strtotime($dates['start_date'])).' au '.strftime("%d %B", strtotime($dates['end_date']));
							}
							echo '<br/>';
						...
						if (Kidzou_Geoloc::has_post_location()) {
							$location = Kidzou_Geoloc::get_post_location();
							echo '<i class="fa fa-map-pin"></i>&nbsp;'.$location['location_city'];
						}
					?>
					</p>
					<div class="post-meta vcard">
