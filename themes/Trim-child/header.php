				...
				<div id="top-area">
					...
					<div class="top">
						...
						<!--span class="kz-button"-->
							...
							<a href="#" class="share">Restez inform&eacute;s</a>&nbsp;|&nbsp;<br/>
						<!--/span-->
						...
						<div class="search">
							<form method="get" id="searchform" action="<?php echo esc_url( kz_geo_home_url(  ) ); ?>/">
								<!-- <label for="searchinput">Recherchez sur Kidzou :</label> -->
								<input type="search" placeholder="<?php esc_attr_e('On va o&ugrave; ?', 'Trim'); ?>" name="s" id="searchinput" title="Entrez des mots tels que week-end ou vacances"   /> 
					...
					<a href="<?php echo esc_url( kz_geo_home_url( ) ); ?>">
					...
					if (count($villes)>1) { //si une seule ville, on n'affiche pas le choix
						
						foreach ($villes as $key => $value) {
							$isSelected = (kz_get_request_metropole()==$value->slug ? true : false );
					        echo '<a class="'.($isSelected ? 'readmore' : 'metropole').'" data-metropole="'.$value->slug.'" href="'.home_url("/").$value->slug.'">'.$value->name.'</a>&nbsp;';
					    }
							...
							if ($primaryNav == '') { ?>
								...
								<ul class="<?php echo esc_attr( $menuClass ); ?>">
									<?php if (get_option('trim_home_link') == 'on') { ?>
										<li <?php if (is_home()) echo('class="current_page_item"') ?>><a href="<?php echo esc_url( kz_geo_home_url(  ) ); ?>"><?php esc_html_e('Home','Trim') ?></a></li>
									<?php }; ?>
