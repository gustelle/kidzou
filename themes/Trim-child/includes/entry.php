...
<?php
	...
	if ( is_single() || is_category() || is_tax() || is_home() || is_search() || is_archive()) {

		if ( has_term('','category') && function_exists( 'kz_is_post' ) && kz_is_post() ) {
		...
		else if (function_exists( 'kz_is_event' ) && kz_is_event()) {
			$parentsIDArray = array('evenement' );
			$parentsNameArray = array('Ev&egrave;nement');
		}

		else if (function_exists( 'kz_is_offre' ) && kz_is_offre()) {
			$parentsIDArray = array('offres' );
			$parentsNameArray = array('Offre');
		}

		else if (function_exists( 'kz_is_concours' ) && kz_is_concours()) {
			$parentsIDArray = array('concours' );
			$parentsNameArray = array('Jeux concours');
		}

	}
?>

	<article class="clearfix <?php echo isset($parentsIDArray) ? $parentsIDArray[0] : ''; ?>" itemscope itemtype="http://schema.org/BlogPosting">
		...
		<a href="<?php the_permalink(); ?>">
			...
			<?php echo '<span class="meta '.(isset($parentsIDArray) ? $parentsIDArray[0] : '').'">'.(isset($parentsNameArray) ? $parentsNameArray[0] : '').'</span>'; ?>
		</a>
		</div><!-- .entry-thumbnail -->
